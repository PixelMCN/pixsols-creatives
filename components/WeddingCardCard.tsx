...
interface WeddingCardCardProps {
    card: WeddingCard;
    index: number;
    onPreview: (card: WeddingCard) => void;
}

export default function WeddingCardCard({
    card,
    index,
    onPreview,
}: WeddingCardCardProps) {
    return (
        <motion.div
        ...
        >
            {/* Thumbnail */}
            <div className="relative aspect-[3/4] bg-surface border border-border overflow-hidden hover:-translate-y-2 transition-transform duration-300">
                <Image
                    src={card.thumbnail}
                    alt={card.name}
                    fill
